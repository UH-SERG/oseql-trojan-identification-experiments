...
def calc_asr(preds_on_clean_path, preds_on_poisoned_path):
    ...
    with open(preds_on_clean_path, 'r') as preds_on_clean_file:
        for line in preds_on_clean_file:
            ...
            if len(fields) == 3:
                id1, id2, pred_on_clean = fields
                sample_id = str(min(int(id1), int(id2))) + "_" + str(max(int(id1), int(id2)))
                preds_on_clean[sample_id] = int(pred_on_clean)  # Convert to integer
                if preds_on_clean[sample_id] == 1:
    ...
    with open(preds_on_poisoned_path, 'r') as preds_on_poisoned_file:
        for line in preds_on_poisoned_file:
            fields = line.strip().split()
            if len(fields) == 3:
                id1, id2, pred_on_poisoned = fields
                sample_id = str(min(int(id1), int(id2))) + "_" + str(max(int(id1), int(id2)))
                preds_on_poisoned[sample_id] = int(pred_on_poisoned)  # Convert to integer
